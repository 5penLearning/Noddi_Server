...
public class OpenAiClient {
    ...
    static final String ISSUE_EXTRACTION_RULES = """
            - 주요 이슈는 회의에서 현재 문제로 언급된 오류, 장애, 지연, 위험, 우려 사항, 제약 사항, 통제 상황 또는 미해결 쟁점이다.
            - 회의·보고·발표·뉴스 형식과 관계없이 전문에 현재의 문제나 위험이 등장하면 issues에 포함한다.
            - 문제의 담당자, 해결 방법 또는 마감일이 정해졌더라도 문제 자체가 언급됐다면 issues에 포함한다.
            - 안전요원 배치, 임시 통제, 조사 착수, 대응 예정, 수정 진행 중은 해결 완료가 아니라 대응 중인 상태이므로 issues에서 제외하지 않는다.
            - 전문에서 문제가 완전히 해결됨, 정상화됨, 종료됨이 명시된 경우에만 해결된 문제로 판단하여 제외한다.
            - 하나의 내용이 문제이면서 후속 업무이기도 하면 issues와 actionItems 양쪽에 각각 포함할 수 있다.
            - issues에는 수행할 업무가 아니라 현재 발생한 문제 또는 위험 상태를 작성한다.
            - 단순한 질문, 아이디어, 일반적인 의견, 일정 안내, 확정된 결정사항은 issues에 포함하지 않는다.
            - 동일한 문제가 여러 번 언급되면 하나로 통합한다.
            - 각 이슈는 문제의 대상과 상황이 드러나는 간결한 문장으로 작성한다.
            - 최종 응답 전에 summary에 문제·위험·우려·장애·지연·통제 내용이 있는데 issues가 비어 있지 않은지 반드시 다시 점검한다.
            - 회의 전문에 문제나 위험 요소가 정말 없다면 빈 배열을 반환한다.
            """;
    ...
    public OpenAiResponseDto.MeetingSummary summarizeText(
        ...
        try {
            ...
            String systemPrompt = """
                    ...
                    - 회의에서는 성을 생략하거나 이름 뒤에 님, 씨, 이, 형, 누나, 언니, 오빠 등의 호칭을 붙일 수 있다.
                    - 예를 들어 팀원 목록의 "강상준"은 전문에서 "상준", "상준님", "상준 씨", "상준이", "상준이 형", "상준 형"으로 언급될 수 있다.
                    - 이름에서 호칭을 제거하고 성이 생략된 표현까지 고려했을 때 정확히 한 명의 팀원과 대응되는 경우에만 담당자를 연결한다.
                    - 담당자를 연결할 때 assigneeUserId와 assigneeName에는 전문의 호칭 표현이 아니라 팀원 목록의 정식 userId와 전체 name을 반환한다.
                    - 성이 생략된 이름과 대응되는 팀원이 여러 명이면 특정 사용자를 추측하지 않는다.
        ...
        return Map.of(
                ...
                "properties", Map.of(
                        ...
                        "assigneeName", Map.of(
                                ...
                                "description", "전문의 이름·호칭을 팀원 목록과 연결한 뒤 반환하는 팀원의 정식 전체 이름"
                        ),
                        "dueDate", Map.of(
                                "type", List.of("string", "null"),
    ...
    private Map<String, Object> createMeetingSummarySchema() {
        return Map.of(
                ...
                "properties", Map.of(
                        ...
                        "issues", Map.of(
                                "type", "array",
                                "description", ISSUE_EXTRACTION_RULES,
                                "items", Map.of(
                                        "type", "string",
                                        "description", "수행 업무가 아닌 현재 문제 또는 위험 상태를 나타내는 문장"
                                )
                        ),
                        "actionItems", Map.of(
                                "type", "array",
